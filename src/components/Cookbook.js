...
function Cookbook() {
  return (
    <>
      <h1 style = {{color:'gray'}}>Cook-Book</h1>
      <ol>
        { recipes.map(({name, ingredients, instructions}) => 
            <li>{name} : <br style={{padding : '10rem'}}/> Ingredients : {ingredients.map(el => <ul><em style={{color : 'green' , fontSize : '10px'}}>{el}</em></ul>)} <br /> Instructions : {instructions.map(el => <ul><em style={{color : 'blue' , fontSize : '12px', display : 'inline-block', textAlign: 'left'}}>{el}</em></ul>)}<br /></li>)
        }
      </ol>
    </>
  )
...
// {name} <br /> Ingredients : {ingredients.map(el => el)} <br /> Instructions : {instructions.map(el => el)}
